...
exports.userController = exports.uploadMiddleware = void 0;
...
const getUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const { data: { user }, } = yield db_config_1.supabase.auth.getUser(jwt);
        if (!user) {
        ...
        if (!listError && files && files.length > 0) {
            ...
            if (userFile) {
                const { data: urlData } = db_config_1.supabase.storage
                    .from("userimages")
                    .getPublicUrl(`images/${user.id}`);
                publicUrl = urlData.publicUrl;
            }
        }
        let { data: levels, error } = yield db_config_1.supabase
            .from("levels")
            .select("exp")
            .eq("user_id", user.id);
        if (error)
        ...
        return res.status(200).json(Object.assign(Object.assign({ user }, (publicUrl && { publicUrl })), (levels && levels.length > 0 && { exp, level })));
    }
    catch (error) {
...
const updateUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const { data: responseData, error } = yield db_config_1.supabase.auth.updateUser({
            email: email && email,
...
const uploadUserProfilePicture = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const { data, error } = yield db_config_1.supabase.storage
            .from("userimages")
